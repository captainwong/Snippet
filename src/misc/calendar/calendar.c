...
static int mdayOfTerm(int y, int n) {
	...
	snprintf(buf, sizeof(buf), "%u", chunk);
	int offsets[] = { 0, 1, 3, 4 };
...
const char* heavenlyStemToString(enum HeavenlyStem hs) {
	switch (hs) {
	case HS_JIA:	return "甲";
	case HS_YI:		return "乙";
	case HS_BING:	return "丙";
	case HS_DING:	return "丁";
	case HS_WU:		return "戊";
	case HS_JI:		return "己";
	case HS_GENG:	return "庚";
	case HS_XIN:	return "辛";
	case HS_REN:	return "壬";
	case HS_GUI:	return "癸";
	default:		return "Invalid HeavenlyStem";
	}
}
...
const char* earthlyBranchesToString(enum EarthlyBranch eb) {
	switch (eb) {
	case EB_ZI:		return "子";
	case EB_CHOU:	return "丑";
	case EB_YIN:	return "寅";
	case EB_MAO:	return "卯";
	case EB_CHEN:	return "辰";
	case EB_SI:		return "巳";
	case EB_WU:		return "午";
	case EB_WEI:	return "未";
	case EB_SHEN:	return "申";
	case EB_YOU:	return "酉";
	case EB_XU:		return "戌";
	case EB_HAI:	return "亥";
	default:		return "Invalid EarthlyBranches";
	}
}
...
{
	switch (zodiac) {
	case CZ_RAT:		return "鼠";
	case CZ_OX:			return "牛";
	case CZ_TIGER:		return "虎";
	case CZ_RABBIT:		return "兔";
	case CZ_DRAGON:		return "龙";
	case CZ_SNAKE:		return "蛇";
	case CZ_HORSE:		return "马";
	case CZ_GOAT:		return "羊";
	case CZ_MONKEY:		return "猴";
	case CZ_ROOSTER:	return "鸡";
	case CZ_DOG:		return "狗";
	case CZ_PIG:		return "猪";
	default:			return "Invalid ChineseZodiac";
	}
}
...
const char* astroToString(enum AstrologicalSign astro) {
	switch (astro) {
	case AS_Capricornus:	return "摩羯";
	case AS_Auarius:		return "宝瓶";
	case AS_Pisces:			return "双鱼";
	case AS_Aries:			return "白羊";
	case AS_Taurus:			return "金牛";
	case AS_Gemini:			return "双子";
	case AS_Cancer:			return "巨蟹";
	case AS_Leo:			return "狮子";
	case AS_Virgo:			return "处女";
	case AS_Libra:			return "天秤";
	case AS_Scorpio:		return "天蝎";
	case AS_Sagittarius:	return "射手";
	default:				return "Invalid AstrologicalSign";
	}
}
...
int chineseLunarDateInDiff(const ChineseLunarDate* origin, ChineseLunarDate* dst, int diff)
{
	...
	if (days < 31 || days >= maxDaysSincEpoch) { return 0; }
